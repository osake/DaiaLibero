...
public class Signatur {
    ...
    public Signatur( String solrSignatur ) {
        Matcher matcher = p.matcher( solrSignatur);
        if( matcher.find()) {
                isil = matcher.group(1);
            signatur = matcher.group(2);
        } else {
            throw new IllegalArgumentException("Unknown signatur format.");
        }
    }
